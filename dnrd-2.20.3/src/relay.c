 ...
 *
 * Out:     inf_ptr      - inf_ptr->current contains the server to which to forward the query
 *
...
int handle_query(const struct sockaddr_in *fromaddrp, char *msg, int *len,
		 infnode_t **inf_ptr) //domnode_t **dptr)

{
	...
	*len = replylen;
    ...
    /* get the server list for this interface */
    /* Since interface list is in sorted order, we send request to current server. */
    inf = inf_list->next;

    do
    {
        if (no_srvlist(inf->srvlist)) {
            /* there is no servers for this interface, reply with "Server failure" */
	        log_debug(2, "There is no server for this interface... Trying next"); //Replying to query with \"Server failure\"");
	        //if (!set_srvfail(msg, *len)) return -1;
	        //return 0;
        }
    
        /* find the first active server. This might return null */
        inf->current=NULL;
        inf->current=next_active(inf);
   
        if(inf->current == NULL)
        {
    	    printf("Current interface does not have an active server at the moment - Try next ones if any\n"); 
    	    inf = inf->next;
    	}
    	else
    	    break; // we found a valid current server so just proceed to sending requests
    	    
    } while(inf_list->next != inf->next);
    ...
    if (inf->current) {
      ...
      if ((inf->current->send_time != 0) 
	  && (forward_timeout != 0)
	  && (reactivate_interval != 0)
	  && (now - inf->current->send_time > forward_timeout)) {
	  deactivate_current(inf);
      }
    }

    if (inf->current) {
	log_debug(3, "Forwarding the query to DNS server %s",
		  inet_ntoa(inf->current->addr.sin_addr));
    } else {
    ...
    /* Point the out parameter to correct interface to send request to. */
    *inf_ptr = inf;
    return 1;
}
...
static void reactivate_servers(int interval) {
  ...
  last_try = now;
  do {
    if (!no_srvlist(i->srvlist))
      retry_srvlist(i, interval);
  } while ((i = i->next) != inf_list);  
}
...
static void deactivate_servers(int interval) {
  ...
  do {
    if ((s=i->srvlist)) 
      while ((s=s->next) != i->srvlist) {
        int current_disabled=0;
	if (s->inactive) continue;
	if (s->send_time
	    && (difftime(now, s->send_time) > forward_timeout)) {
	  s->inactive = now;
	  if (s == i->current) /* we need to update i->current */
	    current_disabled=1; 
	}
	if (current_disabled) deactivate_current(i);
      }
  } while ((i = i->next) != inf_list);  
}
...
void run()
{
  ...
  while(1) {
	...
	for(i=0; i<3; ++i)
	{
		if (FD_ISSET(q->next->sock_arr[i], &fdread)) {
	  		udp_handle_reply(q, i);
		}
		
		/* if it's a dummy query used for only reactivation, then break now, because dummy queries
                 * only have a single valid socket.
                 */
		if (q->next->is_dummy == 1)
			break;
	}
      }
